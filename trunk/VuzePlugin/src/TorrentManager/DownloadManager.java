...
public class DownloadManager {
    ...
    private List<VideoFileVO> filterBasic(List<VideoFileVO> listaMovies) {
        ...
        if ((excludeRegex != null) || (excludeRegex.trim().equals(""))) {
            ...
            List<VideoFileVO> listaTemp = listaMovies;
            listaMovies = new ArrayList<VideoFileVO>();

            for (VideoFileVO movieVO : listaTemp) {
    ...
    public void getSubTileForMovie(List<VideoFileVO> listaMovies) {
    ...
    public void getSubTitleForAllCompletedMovies() {
        ...
        List<VideoFileVO> listaMovies = TorrentUtils.getMovieFiles(_pluginInterface);
        // Filtros necessários
        listaMovies = filterBasic(listaMovies);
        if (listaMovies.size() == 0)
            return;
    ...
    private void downloadSubTitles(List<VideoFileVO> listaMovies, IDownloadHandler downloadHandler, DownloadHandlerVO handlerVO) throws Exception {
        ...
        try {
            ...
            for (VideoFileVO movieFileVO : listaMovies) {
                ...
                try {
                    ...
                    _log.fatal(_localeUtil.getLocalisedMessageText(IDownloadHandler.SYSTEM_NAME + ".DownloadingSubtitleError",
